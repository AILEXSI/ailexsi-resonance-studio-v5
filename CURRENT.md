...
| Deps / SBOM | Cargo.toml + `lib.rs` + capabilities use `tauri-plugin-fs` / `tauri-plugin-dialog`. Lockfile now resolves **fs 2.5.2** and **dialog 2.7.3** (same versions as the npm plugins). CycloneDX SBOMs + inventory: `docs/compliance/`. **No LICENSE. No THIRD_PARTY_NOTICES. Licensing is not HUMAN-PROVEN.** |
