...
| Export | Toolbar **Export** öffnet den H.264-MP4-Dialog. Default-Name ist die nächste freie `.vN` (unversioniert = v1 → `.v2`; nach `.v3`…`.v6` → `.v7`). Dedicated **Export WAV**-Button ist weg. `startExport("wav")` existiert intern (Tests/Code), **kein UI-Weg**. |
