 ...
 * Measures 720p30 full export with alternated backends.
 */
import {
  clearFrameSources,
...
async function runOne(identity: "mediabunny" | "ailexsi", job: ReturnType<typeof jobOf>, i: number) {
...
async function timed(identity: "mediabunny" | "ailexsi", job: ReturnType<typeof jobOf>) {
  installWebCodecsProbe();
...
async function repeatBoth(job: ReturnType<typeof jobOf>, warmup: number, measured: number, seed = 0xafe03) {
  ...
  for (let i = 0; i < warmup + measured; i++) {
    ...
    const mb = first === "mediabunny" ? x.ms : y.ms;
    const afe = first === "ailexsi" ? x.ms : y.ms;
    if (i < warmup) {
      mbW.push(mb);
      afeW.push(afe);
    } else {
...
async function rawPass(identity: "mediabunny" | "ailexsi", url: string, times: number[]) {
  installWebCodecsProbe();
  beginAfePerf(identity);
...
try {
  ...
  show("phase export");
  const mbPhase = await timed("mediabunny", { ...job, fileName: "mb-phase.mp4" });
  const afePhase = await timed("ailexsi", { ...job, fileName: "afe-phase.mp4" });

  show("raw 720");
  const mbRaw = await rawPass("mediabunny", srcUrl, seq);
