...
describe("transport chrome (undo / snap)", () => {
  let host: HTMLDivElement | undefined;
  let root: Root | undefined;
  ...
  it("optional undo/redo icons and Snap toggle sit on the transport row", () => {
    host = document.createElement("div");
    document.body.appendChild(host);
    root = createRoot(host);
    const seen: string[] = [];
    act(() => {
      root!.render(
        <Transport
          ...
          onRedo={() => seen.push("redo")}
          onToggleSnap={() => seen.push("snap")}
        />,
      );
    });
    const undo = host.querySelector('[data-testid="transport-undo"]') as HTMLButtonElement;
    const redo = host.querySelector('[data-testid="transport-redo"]') as HTMLButtonElement;
    const snap = host.querySelector('[data-testid="transport-snap"]') as HTMLButtonElement;
    expect(undo.getAttribute("aria-label")).toBe("Undo");
    expect(redo.getAttribute("aria-label")).toBe("Redo");
    expect(snap.textContent?.trim()).toBe("Snap");
    expect(snap.classList.contains("active")).toBe(true);
    expect(undo.textContent?.trim()).not.toBe("Undo");
    expect(redo.textContent?.trim()).not.toBe("Redo");
    act(() => {
      undo.click();
      redo.click();
      snap.click();
    });
    expect(seen).toEqual(["undo", "redo", "snap"]);
  });
});
