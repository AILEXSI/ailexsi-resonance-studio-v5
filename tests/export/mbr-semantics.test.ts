...
function backends(): Array<"mediabunny" | "htmlvideo"> {
  return ["mediabunny", "htmlvideo"];
}

describe("MBR Resonance export semantics (both backends, no decode)", () => {
  ...
  it("Source In/Out + shortened clip + clip rate feed the same sourceTimeSec on both backends", () => {
    const c = exportClip({
      startMs: 0,
      endMs: 1000,
      sourceInMs: 2500,
    ...
    const html = [0, 100, 500, 999].map((t) => sourceTimeSec(c, t, 30));
    expect(html).toEqual(times);
    expect(times[0]).toBeGreaterThanOrEqual(2.5);
    expect(times[3]).toBeLessThan(4.5);
    const nearOut = sourceTimeSec(c, 5000, 30);
    expect(nearOut).toBeLessThan(c.sourceOutMs / 1000);
  });
  ...
  it("VIS fills the gap; black when video is missing; missing-only export still FAILs", () => {
    ...
    expect(missJob.tracks.find((t) => t.id === "V1")!.clips[0]!.missing).toBe(true);
    expect(videoClipAt(missJob, 0)?.missing).toBe(true);
  });

  it("export IN/OUT remaps clip times and source window the same for both backends", () => {
    const p = projectWith(
      [
        clip({
          id: "c1",
