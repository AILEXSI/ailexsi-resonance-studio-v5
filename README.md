...
**E Stem Import:** HUMAN-PROVEN in EXE (alongside D / mixer / save / `.vN` / export). **F Track/Chapter Groups:** HUMAN-PROVEN in EXE (create / assign / collapse / rename; collapse UI only, no group bus). **G Volume Automation:** IMPLEMENTED / AUTOMATED-TESTED only — not HUMAN-PROVEN. **H–N + zettel:** PLANNED / NOT IMPLEMENTED (incl. Track/Mixer Rename, Track Color, Distribute Colors, Relink filename assist — Future UI in `CURRENT.md`, not next slice).
...
Lineage: D/E/save/export EXE = PR #15 tip `234a781` on `cursor/stack-export-vn-1787` (onto PR #14). F EXE = PR #17 feature tip `c4391cb` (`cursor/track-chapter-groups-6442`). G = `cursor/volume-automation-e329` on F tip `9afed5e` (IMPLEMENTED / AUTOMATED-TESTED, not HUMAN-PROVEN, not an EXE SHA). `origin/main` @ `9ceb9bd` (stamp of `0cdcadf`; PR #9 merge `c0392f0`). Version 5.0.0.
...
Production Pass **G**: IMPLEMENTED / AUTOMATED-TESTED (volume automation, VOL lane, linear points, clip gain ≠ static fader ≠ automation). **Not HUMAN-PROVEN.** Linux agent did not build a Windows EXE.
