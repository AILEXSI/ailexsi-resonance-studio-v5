...
- **Speichern unter:** Chrome `showSaveFilePicker` (Ordner + Name, Suggested name `.vN.resonance.json`). Tauri/Exe: nativer Save-Dialog (immer Picker, `defaultPath` versioniert). Firefox ohne FSA: Download. Nach Speichern/Öffnen in der Exe zeigt das File-Panel Dateiname + gemerkten Ordner (`folderRemembered`), nicht „Kein Ordner gemerkt“.
