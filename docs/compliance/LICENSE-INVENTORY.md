...
| --- | --- | --- | --- | --- | --- |
| mediabunny | 1.55.3 | npm runtime | MPL-2.0 | YELLOW | `package.json` + `node_modules/mediabunny/LICENSE` is the standard MPL-2.0 text. Imported from `src/core/exporter/frame-source.ts` (`Input`, `VideoSampleSink`, …). Not vendored; Resonance does not modify mediabunny source in-tree. Vite bundles the imported JS into `dist/assets/*`. The npm `LICENSE` file is **not** automatically copied into `dist/`. |
...
| mediabunny | 1.55.3 | MPL-2.0 | YELLOW |
...
| workspace `ailexsi-resonance-studio-v5` 5.0.0 | 1 | UNKNOWN |
